...
namespace formula_solver {
    ...
    : parser(input_stream, error_stream, n, k){}
    ...
    std::vector<std::vector<int>> FormulaSolver::generate_all_variables_evaluations() const {
        ...
        std::vector<int> current_evaluation(parser.variable_names.size(), 0);

        auto total_combinations = std::pow(parser.number_of_logical_values, parser.variable_names.size());
        ...
        for (int i = 0; i < total_combinations; i++) {
            ...
            for (int position = 0; position < current_evaluation.size(); position++) {
                ...
                if (current_evaluation[position] < parser.number_of_logical_values) {
                    break;
                }
    ...
    std::vector<BinaryTruthTable> FormulaSolver::generate_all_truth_tables() const {
        int num_elements = (parser.number_of_logical_values * (parser.number_of_logical_values + 1)) / 2;
        std::vector<BinaryTruthTable> truth_tables;

        auto total_combinations = std::pow(parser.number_of_logical_values, num_elements);

        for (int index = 0; index < total_combinations; index++) {
            BinaryTruthTable table(parser.number_of_logical_values);
            ...
            for (int i = 0; i < index; ++i) {
                for (int position = 0; position < num_elements; position++) {
                    ...
                    if (cell_values[position] < parser.number_of_logical_values) {
                        break;
                    }
    ...
    void FormulaSolver::find_all_tautological_logical_operators() {
        ...
        int num_of_truth_table_combinations = std::pow(parser.number_of_logical_values, (parser.number_of_logical_values * (parser.number_of_logical_values + 1)) / 2);
        std::vector<int> true_values_in_logic = generate_last_k_values(parser.number_of_logical_values, parser.number_of_true_logical_values);
            ...
            for (int j = num_of_truth_table_combinations - 1; j >= 0; --j) {
                for (int k = num_of_truth_table_combinations - 1; k >= 0; --k) {
                    for (int l = num_of_truth_table_combinations - 1; l >= 0; --l) {
                        ...
                        std::vector<BinaryTruthTable> current_logical_operators = {and_operator, or_operator, implication_operator, equivalence_operator};

                        bool is_tautology = true;

                        for (const auto& evaluation : all_possible_evaluations) {
                            int formula_result = parser.evaluate_formula(evaluation, current_logical_operators);
